...
class ReActExecutor:
    ...
    async def run(self, ctx: ExecutorContext) -> AsyncIterator[bytes]:
        ...
        cancelled = False
        emitter = ctx.emitter
        # Tracked across the whole executor: every inline ``finalize_run``
        # path sets it to True so the outer ``finally`` can close any run
        # that exits otherwise (CancelledError, exception).
        finalized = False
        total_tokens_in = 0
        total_tokens_out = 0
        ...
        try:
            ...
            similarity_threshold = float(
                snapshot.get("similarity_threshold")
                or ctx.agent.similarity_threshold
                or REACT_DEFAULT_SIMILARITY_THRESHOLD
            )
            ...
            try:
                ...
                # Outer ``finally`` (further down) writes a terminal row when no
                # inline path did. We still need to handle the explicit-cancel
                # case here so the SSE stream gets a CANCELED event before we
                # exit the generator.
            ...
            # Last-resort finalize: covers client disconnects
            # (CancelledError / GeneratorExit) and unexpected exceptions.
            # All inline ``finalize_run`` paths flip ``finalized=True``;
            # this guarantees the run never stays ``running`` forever.
            if not finalized:
